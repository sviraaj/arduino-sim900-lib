...
{
if(!at_set())return 0;
if(!sim_status())return 0;
if(!card_pr())return 0;
if(!signal_quality())return 0;
if(!imei_no())return 0;
if(!extended_error_code())return 0;
...
{
	// addr and port should have inherited "" i.e if connecting to google.com, addr="\"www.google.com\""
	gsm_write("AT+CIPSTART=");
	if (mode == 0)gsm_write("\"TCP\",");
	if (mode == 1)gsm_write("\"UDP\",");
	ser.print(addr);
	gsm_write(",");
	ser.print(port);
	gsm_write("\r");
...
uint8_t gsm::http_send_transact(String &url,String &http_type,sd_card *sd_read){
	String post_data = "";
...
uint8_t gsm::http_send_data(String &url,String &http_type,String* post_data)  //base method when doing inheritance
{	
	uint16_t data_length;
	uint16_t time_out = TIME_OUT;
	...
	if(!http_set_para(url))return 0;

	if(post_data!=NULL)
	if(!send_post_data(&data_length,&time_out,post_data))return 0;
...
uint8_t gsm::send_post_data(uint16_t *data_length, uint16_t *time_out, String* post_data){
...
uint8_t gsm::action_parse(String &http_type_got, String &status_code, String &length){
	String received = "";
	uint8_t rec = 0;
	uint8_t temp = 0;
	while (!rec)
	{
		...
		while (ser.available()){
			...
			char c=(char)ser.read();
				received += c;
				if (temp == 1){ if (c != ',')http_type_got += c; else{ temp++;continue; } }
				if (temp == 2){ if (c != ',')status_code += c; else{ temp++;continue; } }
				if (temp == 3 && c == '\n') { rec = 1; break; }
				if (temp == 3){ length += c; }
				if (c == ':') temp++;
		}
...
uint8_t gsm::check_http_status(uint8_t command_http)
{
	...
	case AT_SAPBR:
		...
		while (!rec)
		{
			...
			while (ser.available()){
				...
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				//Serial.println(rec);
				if(rec == 2) rec = 0;              //bug fix to be done in all cases .... remember this.. if rec == 2 and ser 									   //not available, it exists unneccesarily..
				if (rec == 1){ break; }
		...
		else { error_status(AT_SAPBR, response , received); return 0; }
		break;
	...
	case AT_SAPBR_II:
		...
		//DEBUG_PRINT(received);
		//DEBUG_PRINT("\r\n");
		if (response[6] != ':'){ DEBUG_PRINT(F("prob")); error_status(AT_SAPBR_II, response , received); return 0; }
		//DEBUG_PRINT("1");
		//response = "";
		rec = 0;
		expected = "\r\nOK\r\n"; 
	...
	case AT_HTTPINIT:
		...
		while (!rec)
		{
			...
			while (ser.available()){
				...
				//Serial.println(rec);
				if (rec == 1)break;
				//if (rec == 2) { error_status(AT, response); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT_F("\r\n");
		if (response == "OK") return 1;
		else { error_status(AT_HTTPINIT, response , received); return 1; //////////////////CHANGEEEEEEEEEEE
		}
		break;
	...
	case AT_HTTPPARA:
		...
		DEBUG_PRINT_F("\r\n");
		if (response == "OK") return 1;
		else { error_status(AT_HTTPPARA, response, received); return 0; }
		break;
	...
	case AT_HTTPDATA:
		...
		DEBUG_PRINT(response);
		DEBUG_PRINT_F("\r\n");
		if (response = "DOWNLOAD") return 1;  
		else{ error_status(AT_HTTPDATA, response , received); return 0; }	
		break;
	...
	case AT_HTTPDATA_C:
		...
		DEBUG_PRINT_F("\r\n");
		if (response = "OK") return 1;
		else{ DEBUG_PRINT_F("httpdatacontinue badd"); error_status(AT_HTTPDATA, response , received); return 0; }
		response = "";
		rec = 0;
		expected = "\r\n";
	...
	case AT_HTTPACTION:
		...
		DEBUG_PRINT_F("\r\n");
		if (response == "OK") { return 1; }
		else { error_status(AT_HTTPACTION, response , received); return 0; }
		break;
	...
	case AT_HTTPTERM:
		...
		DEBUG_PRINT_F("\r\n");
		if (response == "OK") return 1;
		else { error_status(AT_HTTPTERM, response , received); return 0; }
		break;
...
uint8_t gsm::check_status(uint8_t command)
{
	...
	case AT:
		...
		while (!rec)
		{
			...
			while (ser.available()){
				...
				//Serial.println(rec);
				if (rec == 1)break;
				//if (rec == 2) { error_status(AT, response , received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT_F("\r\n");
		if (response == "OK") return 1;
		else { error_status(AT, response , received); return 0; }
		break;
	...
	case AT_CPIN:
		...
		while (!rec)
		{
			...
			while (ser.available()){
				...
				//Serial.println(rec);
				if (rec == 1){ break; }
				if (rec == 2) { error_status(AT_CPIN, response, received); return 0; }
				//if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		while (!rec)
		{
			while (ser.available()){
				...
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 1){ break; }
				//if(rec==2) {error_status(AT_W,response ,received);return 0;} //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}

		}//while !rec
		DEBUG_PRINT(response);
		DEBUG_PRINT("\r\n");
		if (response == "+CPIN: READYOK") return 1;
		else { error_status(AT_CPIN, response , received); return 0; }
		break;
	...
	case AT_CSQ:
		...
		while (!rec)
		{
			...
			while (ser.available()){
				...
				//if(rec==2) {error_status(AT_CSQ,response, received);return 0;} //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
	...
	case AT_COPS:
		...
		while (!rec)
		{
			while (ser.available()){
				received += (char)ser.read();
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				//if(rec==2) {error_status(AT_CSQ,response);return 0;} //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
	...
	case AT_CGSN:
		...
		while (!rec)
		{
			while (ser.available()){
				...
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 1){ break; }
				if (rec == 2) { error_status(AT_CGSN, response,received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
	...
	case AT_CMEE:
		...
		while (!rec)
		{
			while (ser.available()){
				received += (char)ser.read();
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 2) { error_status(AT_CMEE, response, received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT("\r\n");
		if (response == "OK") return 1;
		else { error_status(AT_CMEE, response, received); return 0; }
		break;
	...
	case AT_CGATT:
		...
		while (!rec)
		{
			while (ser.available()){
				received += (char)ser.read();
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 2) { error_status(AT_CGATT, response , received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
	...
	case AT_CIPSTATUS:
		...
		DEBUG_PRINT("\r\n");
		//delay(1000);
		if (response == "OKSTATE: IP INITIAL")return IP_INITIAL;
		else return 1;
		//else {error_status(AT_CGATT,response, received);return 0;}
		break;
	...
	case AT_SHUT:
		...
		while (!rec)
		{
			while (ser.available()){
				...
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 1){ break; }
				if (rec == 2) { error_status(AT_SHUT, response, received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT("\r\n");
		//delay(1000);
		if (response == "SHUT OK")return 1;
		else { error_status(AT_SHUT, response, received); return 0; }
		break;
	...
	case AT_CIPMUX:
		...
		while (!rec)
		{
			while (ser.available()){
				received += (char)ser.read();
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 2) { error_status(AT_CIPMUX, response, received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT("\r\n");
		if (response == "OK")return 1;
		else { error_status(AT_CIPMUX, response, received); return 0; }
		break;

	case AT_CSTT:
		DEBUG_PRINT("AT+CSTT=\"www\",\"\",\"\"\r\r\n");
		expected = "AT+CSTT=\"www\",\"\",\"\"\r\r\nOK\r\n"; //come back later TODO
		while (!rec)
		{
			while (ser.available()){
				received += (char)ser.read();
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 2) { error_status(AT_CSTT, response, received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT("\r\n");
		if (response == "OK")return 1;
		else { error_status(AT_CSTT, response, received); return 0; }
		break;
	...
	case AT_CIICR:
		...
		while (!rec)
		{
			...
			while (ser.available()){
				...
				//Serial.println(rec);
				if (rec == 1)break;
				if (rec == 2) { error_status(AT_CIICR, response, received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT("\r\n");
		if (response == "OK")return 1;
		else { error_status(AT_CIICR, response , received); return 0; }
		break;
	...
	case AT_CIFSR:
		...
		while (!rec)
		{
			while (ser.available()){
				received += (char)ser.read();
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				//if(rec==2) {error_status(AT_CIFSR,response, received);return 0;} //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
	...
	case AT_CIPSRIP:
		...
		while (!rec)
		{
			while (ser.available()){
				received += (char)ser.read();
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 2) { error_status(AT_CIPSRIP, response, received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT("\r\n");
		if (response == "OK")return 1;
		else { error_status(AT_CIPSRIP, response, received); return 0; }
		break;
	...
	case AT_CIPSTART:
		...
		while (!rec)
		{
			while (ser.available()){
				...
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 1)break;
				//if(rec==2) {error_status(AT_CIPSTART,response , received);return 0;} //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		DEBUG_PRINT(response);
		DEBUG_PRINT("\r\n");
		if (response == "OKCONNECT OK")return 1;
		else { error_status(AT_CIPSTART, response , received); return 0; }
		break;
	...
	case AT_CIPSEND:
		...
		while (!rec)
		{
			while (ser.available()){
				...
				//Serial.print("received:");
				//Serial.println(received);
				if (received[received.length() - 1] == '>') { received += '\r'; }
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 1){ break; }
				if (received[received.length() - 1] == '>') { received += '\n'; }
				if (rec == 2) { error_status(AT_CIPSEND, response , received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}
		...
		else { error_status(AT_CIPSEND, response , received); return 0; }
		break;
	...
	case AT_CIPRECV:
		...
		while (!rec)
		{
			while (ser.available()){
				...
				rec = str_parse(expected, received, response); //gives zero i.e. false if serial receive not ended.
				if (rec == 1){ break; }
				if (rec == 2) { error_status(AT_CIPRECV, response , received); return 0; } //if response  is null string then the returned command itself is not right i.e. the sim900A is not communicating properly.
			}

		}//while !rec
		DEBUG_PRINT(response);
		DEBUG_PRINT("\r\n");
		if (response == "SEND OK")return 1;
		else { error_status(AT_CIPRECV, response , received); return 0; }
		break;
...
uint8_t gsm::str_parse(String &expected, String &received, String &response)
{
	...
	if (n == 0){
		for (int i = 0; i<received.length(); i++){
			...
			if (expected[i] != received[i]);//return 2;    if return 2 take care in all cases..
		}
		return 0;
	}

	if (resp == 1){
		if (received[received.length() - 1] == '\r')resp = 0;
		else{
