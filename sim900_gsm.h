#ifndef __SIM900_GSM_H__
#define __SIM900_GSM_H__
...
public:
    ...
    uint8_t connect_tcp(uint8_t mode, const String &addr,
                        const String &port);                 // tcp conn
    ...
    uint8_t http_send_data(String &url, String &http_type,
                           String *post_data);  // http conn
    uint8_t http_send_transact(String &url, String &http_type,
                               sd_card *sd_read);  // http conn
    uint8_t send_post_data(uint16_t *data_length, uint16_t *time_out,
                           String *post_data);  // http conn
...
private:
    ...
    uint8_t action_parse(String &http_type_got, String &status_code,
                         String &length);
